...
def weight_edge(x, edge_index):
    edge_weight = torch.ones(edge_index.shape[1], device=x.device)
    dim_x = x[0].shape[0]
    for i in range(edge_index.shape[1]):
        node_u = edge_index[0][i]
        node_v = edge_index[1][i]
        edge_weight[i] = F.pairwise_distance(x[node_u].reshape(1, dim_x), x[node_v].reshape(1, dim_x), p=2)
    
    return edge_weight

def get_energy_softmax(x, edge_index, resistance, batch, epoch, beta):
    alpha = 1 - beta
    raw, _ = edge_index
    edge_batch = batch[raw]
    dim_x = x[0].shape[0]
    energy = torch.ones(edge_index.shape[1], device=x.device)
    for graph in range(max(edge_batch) +1):
        scale = torch.where(edge_batch==graph)
        num_edges = scale[0].shape[0]
        u = edge_index[0][scale]
        v = edge_index[1][scale]
        temp = F.pairwise_distance(x[u].reshape(num_edges, dim_x), x[v].reshape(num_edges, dim_x), p=2)
        energy[scale] = F.normalize(temp.reshape([1,num_edges]))
        energy[scale] = (resistance[scale] / energy[scale]).sigmoid()
    
    threshold = torch.quantile(energy, alpha)
    new_edge_index = edge_index.T[torch.where(energy < threshold)].T
    boundary_x = x[edge_index.T[torch.where(energy >= 1)].T]

    return new_edge_index, boundary_x
